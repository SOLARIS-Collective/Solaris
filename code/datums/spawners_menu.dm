		...
		for(var/spawner_obj in GLOB.mob_spawners[spawner])
			...
			if(!this["desc"])
				if(istype(spawner_obj, /obj/effect/mob_spawn))
					...
					this["can_load_appearance"] = MS.can_load_appearance // [MANKIND-EDIT] - MANKIND_LOAD_PREF
				else
...
/datum/spawners_menu/ui_act(action, params)
	...
	if(!istype(MS) || !(MS in GLOB.poi_list)) // [MANKIND-EDIT] - if(!istype(MS) || !(MS in SSpoints_of_interest.other_points_of_interest))
		return
	switch(action)
		if("jump")
			if(MS)
