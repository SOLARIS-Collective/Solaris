...
/datum/ai_controller/proc/able_to_run()
	if(world.time < paused_until)
		return FALSE
	// [MANKIND-ADD] - FIXES_RUNTIMES - Проверяем, что pawn еще существует и не мертв
	if(QDELETED(pawn))
		return FALSE
	if(isliving(pawn))
...
/datum/ai_controller/process(seconds_per_tick)
	if(!able_to_run())
		// walk(pawn, 0) //stop moving	// [MANKIND-REMOVE] - FIXES_RUNTIMES - Смещено вниз
		if(isliving(pawn))
			var/mob/living/living_pawn = pawn
			if(living_pawn.stat == DEAD && ai_status == AI_STATUS_ON)
				// [MANKIND-ADD] - FIXES_RUNTIMES - Полностью останавливаем движение при смерти
				...
				// [/MANKIND-ADD]
				set_ai_status(AI_STATUS_OFF)
				return	// [MANKIND-ADD] - FIXES_RUNTIMES
		walk(pawn, 0) // [MANKIND-ADD] - FIXES_RUNTIMES - stop moving
		// [/MANKIND-EDIT]
...
/datum/ai_controller/proc/set_ai_status(new_ai_status)
	...
	switch(ai_status)
		...
		if(AI_STATUS_OFF)
			...
			SSai_controllers.active_ai_controllers -= src
			// [MANKIND-ADD] - FIXES_RUNTIMES - Полностью очищаем состояние при отключении
			if(pawn)
				walk(pawn, 0)
				if(isliving(pawn))
					var/mob/living/living_pawn = pawn
