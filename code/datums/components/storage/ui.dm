...
/datum/component/storage/proc/orient2hud_legacy(mob/user, maxcolumns)
	...
	if(islist(numbered_contents))
		for(var/type in numbered_contents)
			...
			// [MANKIND-ADD] - FIXES_ICON_OUT_OF_BORDER - Убеждаемся, что координаты находятся в допустимых пределах
			...
			// [/MANKIND-ADD]
			// [MANKIND-ADD] - FIXES_ICON_OUT_OF_BORDER - Сбрасываем позицию элемента для предотвращения смещения
		...
		for(var/obj/O in accessible_items())
			...
			var/safe_cy = clamp(cy, screen_start_y, screen_start_y + rows - 1)
			// [/MANKIND-ADD]
			// [MANKIND-EDIT] - FIXES_ICON_OUT_OF_BORDER
