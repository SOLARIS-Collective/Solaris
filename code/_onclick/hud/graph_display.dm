 ...
 *
 * Ported from tgstation PR #94487 (Smooth Movement 5.4), adapted to our older
 * hud/plane systems. Verb timing modes render empty until verb tracking exists.
 */
...
/atom/movable/screen/graph_display/bars/cpu_display/proc/refresh_thresholds()
	...
	switch(display_mode)
		...
		if(USAGE_DISPLAY_COMPLETE_CPU)
			...
			// [SOLARIS] - no verb tracking, verb_timings slots are null
			var/verb_boundary = tick_info.verb_timings[last_index]
			if(islist(verb_boundary) && length(verb_boundary))
				boundaries += verb_boundary[1]
...
/atom/movable/screen/graph_display/bars/cpu_display/proc/set_max_display(max_displayable_cpu)
	src.max_displayable_cpu = max(max_displayable_cpu, 1)
...
/atom/movable/screen/graph_display/bars/cpu_display/value_to_height(value)
	return LERP(0, height, clamp(value / max(max_displayable_cpu, 1), 0, 1))
