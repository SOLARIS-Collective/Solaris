...
/datum/controller/subsystem/communications/proc/make_announcement(mob/living/user, is_silicon, input, auth_id)     //WS Edit - Make cap's announcement use logged-in name
	if(!can_announce(user, is_silicon))
		return FALSE
	if(is_silicon)
		priority_announce(html_decode(input), "[user.name] Announces:", 'sound/ai/attention.ogg', "Silicon", has_important_message = TRUE)
		silicon_message_cooldown = world.time + COMMUNICATION_COOLDOWN_AI
	else
		priority_announce(html_decode(user.treat_message(input)), "[user.name] Announces:", 'sound/misc/announce.ogg', "CommunicationsConsole", auth_id, user.virtual_z(), has_important_message = TRUE) //WS Edit - Name on announcements & Make cap's announcement use logged-in name
