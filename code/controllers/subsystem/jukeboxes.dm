...
/datum/controller/subsystem/jukeboxes/fire()
	for(var/list/jukeinfo as anything in activejukeboxes)
		...
		if(!istype(juketrack))
			...
			continue
		// [MANKIND-EDIT] - FIXES_JUKEBOX - Убираем строгость, чтобы можно было обращаться и к другим подобным жукбоксам объектам
		...
		if(above_turf && istransparentturf(above_turf))
			// virtual_ids += above_turf.virtual_z	// [MANKIND-REMOVE] - FIXES_JUKEBOX - Попытка починить баг со слышимостью на соседних вирт. уровнях
			areas += get_area(above_turf)
		if(below_turf && istransparentturf(below_turf))
			// virtual_ids += below_turf.virtual_z	// [MANKIND-REMOVE] - FIXES_JUKEBOX - Попытка починить баг со слышимостью на соседних вирт. уровнях
