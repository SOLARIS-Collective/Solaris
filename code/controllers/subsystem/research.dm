...
SUBSYSTEM_DEF(research)
	...
	var/list/techweb_nodes_experimental = list()	//Node ids that are exclusive to the BEPIS. //PENTEST REVERT
...
/datum/controller/subsystem/research/proc/calculate_techweb_nodes()
	...
	for(var/node_id in techweb_nodes)
		...
		if(node.experimental) //PENTEST REVERT - BEPIS
			techweb_nodes_experimental[node.id] = TRUE //PENTEST REVERT - BEPIS
		CHECK_TICK
