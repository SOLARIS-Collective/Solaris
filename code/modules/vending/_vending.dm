...
/obj/machinery/vending
	...
	*/
	...
	// [MANKIND-ADD] - VENDING_CASH - Добавляем вендингам аккаунт для денег
...
/obj/machinery/vending/Initialize(mapload)
	...
	// [MANKIND-EDIT] - MANKIND_BALANCE_VENDING - Машины на аванпостах сохранят значение all_items_free = TRUE если оно было установлено на карте
...
/obj/machinery/vending/atom_break(damage_flag)
	...
	while (found_anything)
		...
		for(var/record in shuffle(product_records))
			// [MANKIND-ADD] - MANKIND_BALANCE - Производители автоматов торговых теряют огромные деньги из-за вандалов-халявщиков
			if (prob(20))
				say("Bzzzzzzzz...")
				found_anything = FALSE
				break
...
/obj/machinery/vending/proc/update_canister()
	if (!component_parts)
		return
	// [MANKIND-ADD] - MANKIND_BALANCE - Проверка на лигитимную разборку
	if (valid == FALSE)
		return
	valid = FALSE
	// [/MANKIND-ADD]
...
/obj/machinery/vending/crowbar_act(mob/living/user, obj/item/I)
	if(!component_parts)
		return FALSE
	// [MANKIND-ADD] - MANKIND_BALANCE - Да, разборка была с помощью инструмента
	valid = TRUE
	// [/MANKIND-ADD]
	...
	switch(action)
		if("vend")
			...
			if (R.amount <= 0 && R.max_amount >= 0)
				...
				vend_ready = TRUE
				return
			//[MANKIND_EDIT] - VENDING_CASH - Перенёс определение настоящей цены выше
			...
			if(!all_items_free && ishuman(usr))
				...
				// [MANKIND-EDIT] - VENDING_CASH - Добавил собственный счёт в вендомат
				...
				// if(!C) // ORIGINAL
					...
					return
				// [MANKIND-EDIT] - VENDING_CASH - Добавил собственный счёт в вендомат
				// else if (!C.registered_account && !mining_point_vendor) // ORIGINAL
				...
				if(mining_point_vendor)
					...
					// SSblackbox.record_feedback("amount", "vending_spent", price_to_use)	// ORIGINAL
					// var/datum/bank_account/account = C.registered_account
					// if(price_to_use)
					...
					// [MANKIND-EDIT] - VENDING_CASH - Перенёс определение настоящей цены выше
					/* 	var/datum/bank_account/account = C.registered_account
						if(coin_records.Find(R) || hidden_records.Find(R))
							price_to_use = R.custom_premium_price ? R.custom_premium_price : extra_price
					*/	// ORIGINAL
			...
			vend_ready = TRUE
		// [MANKIND-ADD] - VENDING_CASH Добавил снятие денег с собственного счёта
		if("withdrawCash")
			. = TRUE
...
/obj/machinery/vending/proc/throw_item()
	...
	// [MANKIND-EDIT] - MANKIND_BALANCE - Чиним абьюз
...
/obj/machinery/vending/custom
	...
	// [MANKIND-REMOVE] - MANKIND_FIXES - Перемещено в родителя через модульность
