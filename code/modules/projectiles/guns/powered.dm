...
/obj/item/gun/ballistic/automatic/powered/attackby(obj/item/A, mob/user, params)
	...
	if (!internal_cell && (A.type in allowed_cell_types))
		...
		if (!cell)
			insert_cell(user, C)
		// [MANKIND-ADD] - MANKIND_QOL - Позволяет удобно заменить батарею другой батареей.
		else
			if (tac_reloads)
...
/obj/item/gun/ballistic/automatic/powered/proc/eject_cell(mob/user, obj/item/stock_parts/cell/gun/tac_load = null)
	...
	update_appearance()
	// [MANKIND-ADD] - MANKIND_QOL - Замена батереи рукой занимает 3.5 секунды, отвёрткой 1.5 секунды и чем лучше отвёртка - тем быстрее достанется батарея.
	if(user)
...
/obj/item/gun/ballistic/automatic/powered/screwdriver_act(mob/living/user, obj/item/I)
	if(cell && !internal_cell)
		...
		if(I.use_tool(src, user, 1.5 SECONDS, volume=100)) // [MANKIND-EDIT] "unscrewing_time" заменено на "1.5 SECONDS".
