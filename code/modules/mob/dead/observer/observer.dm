...
/mob/dead/observer
	...
	/// Reference to the live body this ghost spawned from.
	/// Used by the VIP "Reset Ghost" verb to restore the character appearance.
	var/mob/body_src
...
/mob/dead/observer/Initialize()
	...
	animate(src, pixel_y = 2, time = 10, loop = -1, flags = ANIMATION_RELATIVE) //PENTEST EDIT
...
/*
 * Updates the ghost's icon. If the ghost has a mob-based appearance, don't override it.
 */
...
/mob/dead/observer/proc/ManualFollow(atom/movable/target)
	...
	// [MANKIND-ADD] - logging admin actions.
	if(src.client.holder && istype(target, /mob/living/carbon/human))
		var/mob/living/carbon/human/target_human = target
		log_mankind_admin("\[GHOST]: [usr.key] has started orbiting [target_human.key] / [target_human.real_name].")
	// [/MANKIND-ADD]
...
/mob/dead/observer/stop_orbit(datum/component/orbiter/orbits)
	...
	animate(pixel_y = base_pixel_y - 2, time = 1 SECONDS, loop = -1, flags = ANIMATION_RELATIVE) //PENTEST ADDITION
