...
/proc/web_sound(mob/user, input)
	...
	if(istext(input))
		...
		switch(anon)
			if("Yes")
				if(res == "Yes")
					to_chat(world, span_boldannounce("[user.client] played: [webpage_url]"), confidential = TRUE)
				else
					to_chat(world, span_boldannounce("[user.client] played some music"), confidential = TRUE)
			if("No")
				if(res == "Yes")
