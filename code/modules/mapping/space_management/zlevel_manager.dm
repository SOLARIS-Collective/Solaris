...
/datum/controller/subsystem/mapping/proc/add_new_zlevel(name, z_type = /datum/space_level, allocation_type = ALLOCATION_FREE, zlevel_role = null) // PENTEST EDIT - Manditory ZLevels
	...
	if (world.maxz < new_z)
		world.incrementMaxZ()
	var/datum/space_level/S = new z_type(new_z, name, allocation_type, zlevel_role) // PENTEST EDIT - Manditory ZLevels
	z_list += S
	return S
