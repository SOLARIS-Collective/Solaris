...
/proc/voice_of_god(message, mob/living/user, list/span_list, base_multiplier = 1, include_speaker = FALSE, message_admins = TRUE)
	...
	if(user.mind)
		...
		// PENTEST ADDITION - Mimes are bad at speaking with the voice of god
	...
	//SILENCE
		...
		cooldown = COOLDOWN_STUN
		for(var/mob/living/carbon/C in listeners)
			// if(user.mind.assigned_role == "Curator") //PENTEST REMOVAL
			if(user.mind && (user.mind.assigned_role == "Curator" || user.mind.assigned_role == "Mime")) //PENTEST ADDITION - Mimes can't be silenced
				power_multiplier *= 3
