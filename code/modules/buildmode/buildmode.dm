...
/proc/togglebuildmode(mob/M as mob in GLOB.player_list)
	if(M.client)
		if(istype(M.client.click_intercept,/datum/buildmode))
			...
			log_mankind_admin("ADMIN: [key_name(usr)] has left build mode.") // [CELADON-ADD] - logging admin actions.
		else
			new /datum/buildmode(M.client)
			message_admins("[key_name_admin(usr)] has entered build mode.")
			log_admin("[key_name(usr)] has entered build mode.")
			log_mankind_admin("ADMIN: [key_name(usr)] has entered build mode.") // [CELADON-ADD] - logging admin actions.
