...
/obj/item/toy/prize/proc/check_battle_start(mob/living/carbon/user, obj/item/toy/prize/attacker, mob/living/carbon/target)
	if(attacker && attacker.in_combat)
		...
		target && to_chat(target, span_notice("Your [attacker.name] is in combat."))
		return FALSE
	if(in_combat)
		to_chat(user, span_notice("Your [name] is in combat."))
		target && to_chat(target, span_notice("[target.p_their()] [name] is in combat."))
		return FALSE
	if(attacker && attacker.timer > world.time)
		...
		target && to_chat(target, span_notice("Your [attacker.name] isn't ready for battle."))
		return FALSE
	if(timer > world.time)
		to_chat(user, span_notice("Your [name] isn't ready for battle."))
		target && to_chat(target, span_notice("[target.p_their()] [name] isn't ready for battle."))
		return FALSE
