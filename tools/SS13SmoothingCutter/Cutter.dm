...
/mob/verb/ChooseDMI(dmi as file)
	...
	if(isfile(dmifile) && (copytext("[dmifile]",-4) == ".dmi"))
		...
		to_chat(world, "<font color=red>Bad DMI file '[dmifile]'</font>")
...
/proc/SliceNDice(dmifile as file)
	...
	switch(states.len)
		...
		if(STATE_COUNT_NORMAL)
			to_chat(world, "4 States, running in Non-Diagonal mode")
		if(STATE_COUNT_DIAGONAL)
			to_chat(world, "5 States, running in Diagonal mode")
	...
	for(var/state in states)
		...
		switch(statename)
			if("box")
				...
				to_chat(world, "Box: \icon[box] -> \icon[corner1i] \icon[corner2i] \icon[corner3i] \icon[corner4i]")
			...
			if("line")
				...
				to_chat(world, "Line: \icon[line] -> \icon[line1n] \icon[line2n] \icon[line3s] \icon[line4s] \icon[line1w] \icon[line2e] \icon[line3w] \icon[line4e]")
			...
			if("center_4")
				...
				to_chat(world, "Center4: \icon[center4] -> \icon[corner1nw] \icon[corner2ne] \icon[corner3sw] \icon[corner4se]")
			...
			if("center_8")
				...
				to_chat(world, "Center8: \icon[center8] -> \icon[corner1f] \icon[corner2f] \icon[corner3f] \icon[corner4f]")
			...
			if("diag")
				...
				to_chat(world, "Diag: \icon[diag] -> \icon[diagse] \icon[diagsw] \icon[diagne] \icon[diagnw]")
			...
			if("diag_corner_a")
				...
				to_chat(world, "Diag_Corner_A: \icon[diag_corner_a] -> \icon[diagse0] \icon[diagsw0] \icon[diagne0] \icon[diagnw0]")
			...
			if("diag_corner_b")
				...
				to_chat(world, "Diag_Corner_B: \icon[diag_corner_b] -> \icon[diagse1] \icon[diagsw1] \icon[diagne1] \icon[diagnw1]")



		fcopy(outputIcon, filename)	//Update output icon each iteration
	to_chat(world, "Finished [filename]!")
