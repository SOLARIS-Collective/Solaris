...
Одним из решений проблемы - основать наш сервер на надежной кодовой базе, которая в основном поддерживается кем-то другим, в нашем случае - [Shiptest](https://github.com/MANKIND-Collective/Pentest), и вставлять наш контент модульным образом, следуя общему направлению кода (но не геймплея) апстрима, отражая любые его изменения.
...
Далее тебе нужно создать папку, название которой - ID твоего мода строчными буквами. Например, для `HELLO_WORLD` это будет путь `mod_mankind/hello_world/`
...
Инициализируются модпаки на этапе `SS_INIT_EARLY` с вызовом соответствующей процедуры у синглтона. Подробнее можно почитать в [`mod_mankind/_modpack.dm`](/mod_mankind/_modpack.dm)
...
**Для любой системы** с установленным Python 3 - запустить файл `mod_mankind/CREATE_MOD.py`.

В **Windows** можно это сделать ещё двумя способами:
1. Открыть конмандную строку в папке Shiptest и прописать:
    ```bat
    powershell mod_mankind\CREATE_MOD.ps1
    ```
2. Открыть PowerShell в папке Shiptest и прописать:
    ```ps
    .\mod_mankind\CREATE_MOD.ps1
    ```

Те, кто пользуется **Linux или WSL** могут использовать bash файл:
```bash
./mod_mankind/CREATE_MOD.sh
```

#### Вручную
Чтобы реализовать основу мода достаточно выполнить три действия:
- Создать папку `mod_mankind/hello_world/`.
- Скопировать в папку `mod_mankind/hello_world/` все файлы из [`/mod_mankind/_example/`](/mod_mankind/_example/).
- Переименовать `example.dme` и `example.dm` в `_hello_world.dme` и `_hello_world.dm`.

### Включение модпака в билд
После того как модпак будет создан или сделан, его необходимо включить в главном файле `mod_mankind.dme` модульной папки! Именно в ней прописываются все пути до всех файлов с расширением `.dme`.
...
```dm
#include "mod_mankind/hello_world/_hello_world.dm"
```
А вот так:
```dm
...
`code/`:
- **✅ Правильно:** `/mod_mankind/hello_world/code/disease_mob.dm`
- **❌ Неправильно:** `/mod_mankind/hello_world/code/modules/antagonists/disease/disease_mob.dm`

`icons/`:
- **✅ Правильно:** `/mod_mankind/hello_world/icons/mining_righthand.dmi`
- **❌ Неправильно:** `/mod_mankind/hello_world/icons/mob/inhands/equipment/mining_righthand.dmi`
...
Например: `code/modules/mob/living/living.dm` → `mod_mankind/_master_files/code/modules/mob/living/living.dm`
...
- **Удаление (множество строк)**
  ...
  ```dm
  // [MANKIND-REMOVE] - SHUTTLE_TOGGLE - Перемещено в /mod_mankind/shuttle_toggle/randomverbs.dm
...
## Дефайны (`#define`)
Из-за того, как BYOND загружает файлы, есть необходимость существования отдельной папки для хранения дефайнов модов. Эта папка - `code/__defines/~mod_mankind/`, в которой ты можешь создать новый файл, либо дописать код в существующий.
