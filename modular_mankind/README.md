...
Одним из решений проблемы - основать наш сервер на надежной кодовой базе, которая в основном поддерживается кем-то другим, в нашем случае - [Pentest](https://github.com/MANKIND-Collective/Pentest), и вставлять наш контент модульным образом, следуя общему направлению кода (но не геймплея) апстрима, отражая любые его изменения.
...
Далее тебе нужно создать папку, название которой - ID твоего мода строчными буквами. Например, для `HELLO_WORLD` это будет путь `modular_mankind/hello_world/`
...
Инициализируются модпаки на этапе `SS_INIT_EARLY` с вызовом соответствующей процедуры у синглтона. Подробнее можно почитать в [`modular_mankind/_modpack.dm`](/modular_mankind/_modpack.dm)
...
```
modular_mankind/_storage_icons
├─ icons
│  ├─ any_icon.dmi
│  ╰─ some_icon.png
├─ _storage_icons.dm
...
```
modular_mankind/_storage_sounds
├─ sound
│  ├─ any_sound.ogg
│  ╰─ some_sound.mp3
├─ _storage_sounds.dm
...
**Для любой системы** с установленным Python 3 - запустить файл `modular_mankind/CREATE_MOD.py`.

В **Windows** можно это сделать ещё двумя способами:
1. Открыть конмандную строку в папке Shiptest и прописать:
    ```bat
    powershell modular_mankind\CREATE_MOD.ps1
    ```
2. Открыть PowerShell в папке Shiptest и прописать:
    ```ps
    .\modular_mankind\CREATE_MOD.ps1
    ```

Те, кто пользуется **Linux или WSL** могут использовать bash файл:
```bash
./modular_mankind/CREATE_MOD.sh
```

#### Вручную
Чтобы реализовать основу мода достаточно выполнить три действия:
- Создать папку `modular_mankind/hello_world/`.
- Скопировать в папку `modular_mankind/hello_world/` все файлы из [`/modular_mankind/_example/`](/modular_mankind/_example/).
- Переименовать `example.dme` и `example.dm` в `_hello_world.dme` и `_hello_world.dm`.

### Включение модпака в билд
После того как модпак будет создан или сделан, его необходимо включить в главном файле `modular_mankind.dme` модульной папки! Именно в ней прописываются все пути до всех файлов с расширением `.dme`.
...
```dm
#include "modular_mankind/hello_world/_hello_world.dm"
```
А вот так:
```dm
...
`code/`:
- **✅ Правильно:** `/modular_mankind/hello_world/code/disease_mob.dm`
- **❌ Неправильно:** `/modular_mankind/hello_world/code/modules/antagonists/disease/disease_mob.dm`

`icons/`:
- **✅ Правильно:** `/modular_mankind/hello_world/icons/mining_righthand.dmi`
- **❌ Неправильно:** `/modular_mankind/hello_world/icons/mob/inhands/equipment/mining_righthand.dmi`
...
Например: `code/modules/mob/living/living.dm` → `modular_mankind/_master_files/code/modules/mob/living/living.dm`
...
Всего есть три основные операции для редактирования кода внутри *кор кода*: ADD, REMOVE, EDIT. Перед ними всегда ставится пометка проекта: MANKIND. Затем через тире ставится ID, к какому модпаку принадлежат изменения. А после, но не обязательно через тире можно оставить комментарий или написать куда были перемещены изменения.

КОНСТРУКЦИИ:
```
	// [MANKIND-ADD] - ID_МОДПАКА - КОММЕНТАРИЙ_НЕОБЯЗАТЕЛЬНЫЙ
	// [MANKIND-REMOVE] - ID_МОДПАКА - КОММЕНТАРИЙ_КУДА_ПЕРЕМЕЩЕНО_ЕСЛИ_ЕСТЬ
	// [MANKIND-EDIT] - ID_МОДПАКА - КОММЕНТАРИЙ_НЕОБЯЗАТЕЛЬНЫЙ
```

Примеры использования:
- **Добавление (одна строка)**
  ```dm
  var/adminEmergencyNoRecall = FALSE	// [MANKIND-ADD] - SHUTTLE_TOGGLE - Причина или комментарий (Необязательно)
  ```
- **Добавление (множество строк)**
  ```dm
  // [MANKIND-ADD] - SHUTTLE_TOGGLE - Причина или комментарий (Необязательно)
...
- **Удаление (одна строка)**
  ```dm
  // var/obj/docking_port/stationary/S	// [MANKIND-REMOVE] - SHUTTLE_TOGGLE - Причина или комментарий (Необязательно)
  ```
- **Удаление (множество строк)**
  ```dm
  // [MANKIND-REMOVE] - SHUTTLE_TOGGLE - Причина или комментарий (Необязательно)
  ...
  ```dm
  // [MANKIND-REMOVE] - SHUTTLE_TOGGLE - Перемещено в /modular_mankind/shuttle_toggle/randomverbs.dm
...
- **Изменение**
  ```dm
  // [MANKIND-EDIT] - SHUTTLE_TOGGLE - Причина или комментарий (Необязательно)
...
## Дефайны (`#define`)
Из-за того, как BYOND загружает файлы, есть необходимость существования отдельной папки для хранения дефайнов модов. Эта папка - `code/__defines/~modular_mankind/`, в которой ты можешь создать новый файл, либо дописать код в существующий.
