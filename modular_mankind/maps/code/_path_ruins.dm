// Hey! Listen! Update _maps\mod_mankind\map_catalogue.txt with your new ruins!

/datum/map_template/ruin/beachplanet
	prefix = "_maps/_mod_mankind/RandomRuins/BeachRuins/"
	ruin_type = RUINTYPE_BEACH
	cost = 1

/datum/map_template/ruin/icemoon
	prefix = "_maps/_mod_mankind/RandomRuins/IceRuins/"
	ruin_type = RUINTYPE_ICE
	cost = 1

/datum/map_template/ruin/jungle
	prefix = "_maps/_mod_mankind/RandomRuins/JungleRuins/"
	ruin_type = RUINTYPE_JUNGLE
	cost = 1

/datum/map_template/ruin/lavaland
	prefix = "_maps/_mod_mankind/RandomRuins/LavaRuins/"
	ruin_type = RUINTYPE_LAVA
	cost = 1

/datum/map_template/ruin/rockplanet
	prefix = "_maps/_mod_mankind/RandomRuins/RockRuins/"
	ruin_type = RUINTYPE_ROCK
	cost = 1

/datum/map_template/ruin/space
	prefix = "_maps/_mod_mankind/RandomRuins/SpaceRuins/"
	allow_duplicates = FALSE
	ruin_type = RUINTYPE_SPACE
	cost = 1

/datum/map_template/ruin/wasteplanet
	prefix = "_maps/_mod_mankind/RandomRuins/WasteRuins/"
	ruin_type = RUINTYPE_WASTE
	cost = 1

/datum/map_template/ruin/whitesands
	prefix = "_maps/_mod_mankind/RandomRuins/SandRuins/"
	ruin_type = RUINTYPE_SAND
	cost = 1

/datum/map_template/ruin/plasma_gaint
	prefix = "_maps/_mod_mankind/RandomRuins/PlasmaRuins/"
	ruin_type = RUINTYPE_PLASMA
	cost = 1

/datum/map_template/ruin/anima
	prefix = "_maps/_mod_mankind/RandomRuins/AnimaRuins/"
	ruin_type = RUINTYPE_ANIMA
	cost = 1

/datum/map_template/ruin/asteroid
	prefix = "_maps/_mod_mankind/RandomRuins/AsteroidRuins/"
	cost = 1
	ruin_type = RUINTYPE_ASTEROID

/datum/map_template/ruin/reebe
	prefix = "_maps/_mod_mankind/RandomRuins/ReebeRuins/"
	ruin_type = RUINTYPE_YELLOW
	allow_duplicates = FALSE
	cost = 5

/datum/map_template/ruin/moon
	prefix = "_maps/_mod_mankind/RandomRuins/MoonRuins/"
	ruin_type = RUINTYPE_MOON
	cost = 1
/datum/map_template/ruin/water
	prefix = "_maps/_mod_mankind/RandomRuins/WaterRuins/"
	ruin_type = RUINTYPE_WATER
	cost = 1
/datum/map_template/ruin/shrouded
	prefix = "_maps/_mod_mankind/RandomRuins/ShroudedRuins/"
	ruin_type = RUINTYPE_SHROUDED
	cost = 1
/datum/map_template/ruin/desert
	prefix = "_maps/_mod_mankind/RandomRuins/DesertRuins/"
	ruin_type = RUINTYPE_DESERT
	cost = 1
/datum/map_template/ruin/earth_sand
	prefix = "_maps/_mod_mankind/RandomRuins/EarthSandRuins/"
	allow_duplicates = FALSE
	ruin_type = RUINTYPE_EARTH_SAND
	cost = 5
