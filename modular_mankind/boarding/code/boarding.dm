/obj/effect/temp_visual/boarding_alert
	name = "phasing energy"
	icon = 'modular_mankind/_storage_icons/icons/machinery/boarding.dmi'
...
/obj/machinery/boarding_pad/attack_hand(mob/user)
	...
	if(target)
		...
		if(do_after(M, 5 SECONDS, target = src))
			...
			playsound(src, 'modular_mankind/_storage_sounds/sound/boarding_pad.ogg', 60, TRUE, frequency = 1)
			playsound(T, 'modular_mankind/_storage_sounds/sound/boarding_pad.ogg', 60, TRUE, frequency = 1)
