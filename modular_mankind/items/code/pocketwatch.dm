...
/obj/item/pocketwatch/AltClick(mob/user)
	if(!closed)
		...
		playsound(src.loc, 'modular_mankind/_storage_sounds/sound/blade_close.ogg', 50, 1)
	else
		icon_state = "[initial(icon_state)]"
		to_chat(user, "You flip the [name] open.")
		playsound(src.loc, 'modular_mankind/_storage_sounds/sound/blade_open.ogg', 50, 1)
	closed = !closed
