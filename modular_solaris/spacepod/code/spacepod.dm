// SOLARIS-SPACEPOD: адаптация spacepod.dm из PR 2039
...
/obj/spacepod
	...
	icon = 'modular_solaris/_storage_icons/icons/spacepods/construction_2x2.dmi'
	icon_state = "pod_1"
...
/obj/spacepod/update_icon_state()
	...
	if(pod_armor)
		...
		icon = 'modular_solaris/_storage_icons/icons/spacepods/2x2.dmi'
...
/obj/spacepod/update_overlays()
	...
	if(construction_state != SPACEPOD_ARMOR_WELDED)
		if(pod_armor && construction_state >= SPACEPOD_ARMOR_LOOSE)
			var/mutable_appearance/masked_armor = mutable_appearance(icon = 'modular_solaris/_storage_icons/icons/spacepods/construction_2x2.dmi', icon_state = "armor_mask")
	...
	if(atom_integrity <= max_integrity / 2)
		. += image(icon='modular_solaris/_storage_icons/icons/spacepods/2x2.dmi', icon_state="pod_damage")
		if(atom_integrity <= max_integrity / 4)
			. += image(icon='modular_solaris/_storage_icons/icons/spacepods/2x2.dmi', icon_state="pod_fire")
	...
	for(var/cdir in GLOB.cardinals)
		...
		if(left_thrust)
			. += image(icon = 'modular_solaris/_storage_icons/icons/overlay/spacepods/2x2.dmi', icon_state = "rcs_left", dir = cdir)
		if(right_thrust)
			. += image(icon = 'modular_solaris/_storage_icons/icons/overlay/spacepods/2x2.dmi', icon_state = "rcs_right", dir = cdir)
	if(back_thrust)
		var/image/I = image(icon = 'modular_solaris/_storage_icons/icons/overlay/spacepods/2x2.dmi', icon_state = "thrust")
