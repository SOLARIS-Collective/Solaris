// SOLARIS-SPACEPOD: адаптация physics.dm из PR 2039
...
/obj/spacepod/process(time)
	...
	if(velocity_mag || angular_velocity)
		...
		for(var/turf/T in locs)
			...
			drag += 0.001
			var/floating = FALSE
			if(T.has_gravity() && !brakes && velocity_mag > 0.1 && cell && cell.use((is_mining_level(loc) ? 3 : 15) * time))
			...
			if((!floating && T.has_gravity()) || brakes) // brakes are a kind of magboots okay?
				drag += is_mining_level(loc) ? 0.1 : 0.5 // some serious drag. Damn. Except lavaland, it has less gravity or something
